from hicona._core import cooler
...
from .coolbox import get_default_track
from ._dtypes import TrackStyle, TrackModality

if TYPE_CHECKING:
    from .coolbox import PixelsTrack
...
def draw_matrix(
    ...
    *,
    value_cols: str | tuple[str, str] = "count",
    plot_style: TrackStyle = "matrix",
    cmap_specs: TrackModality | tuple[TrackModality, TrackModality] = "counts",
    add_border: bool = False,
    force_palette_split: bool = False,
):
    """Streamlined procedure for pixel matrix plotting."""

    value_cols = (value_cols, value_cols) if isinstance(value_cols, str) else value_cols
    cmap_specs = (cmap_specs, cmap_specs) if isinstance(cmap_specs, str) else cmap_specs

    if (value_cols[0] == value_cols[1]) & (cmap_specs[0] != cmap_specs[1]):
        raise ValueError("Cannot use different colormaps with the same value column.")

    if table_b is None:
        table_b = table_a

    track_a: PixelsTrack = get_default_track(
        table_a,
        value_cols[0],
        modality=cmap_specs[0],
        other_kwargs={"style": plot_style},
    )

    track_b: PixelsTrack = get_default_track(
        table_b,
        value_cols[1],
        modality=cmap_specs[1],
        other_kwargs={"style": plot_style, "orientation": "inverted"},
    )

    # joint_track =
    # TODO: Add TADs option
    # TODO: Add chip seq track option
    # TODO: Add general annotation option
    # TODO: Chr1 now probabily crashes due to coolbox, convert to chr1:start-end
