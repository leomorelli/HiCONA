...
def compute_contrast(graph_a, graph_b, tuning_par):
    graph_c = graph_a.merge(
        graph_b, how="outer", on=["bin1_id", "bin2_id"], suffixes=("_a", "_b")
    )
    del graph_a, graph_b

    graph_c["ab"] = graph_c["count_a"] - graph_c["count_b"]
    group
    # Drop columns?
...
def C_peeling(G, C):
    ...
    # Degree Dictionary
    d = {
        k: [sum(x for x in v if x > 0), abs(sum(x for x in v if x < 0))]  # If equal?
        for k, v in G.items()
    }
