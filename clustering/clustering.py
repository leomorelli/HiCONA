...
def filtered_edges(cool, chromosomes):
    ...
    elist = anno_f[
        ["bin1_id", "bin2_id", "chrom1", "start1", "end1", "chrom2", "start2", "end2"]
    ]

    return elist, bins


def prep_elist(elist, bins: pd.DataFrame):
    ...
    elist_f = pd.concat(
        [elist_f, pd.DataFrame(genomic, columns=["start_code", "end_code", "link"])]
    )
...
def cluster_analysis(graph, c):
    ...
    for i in range(100):
        state = gt.minimize_nested_blockmodel_dl(
            ...
            state_args=dict(
                ...
                state_args=dict(ec=g.ep.link, layers=True, deg_corr=True),
            ),
        )
        states.append(state)
    ...
    state = states[state_index]  # Crashes if loop did not get any positive match
...
def clusters_df(state):
    ...
    for c in groups.columns:
        ...
        new_cat = ["%s" % x for x in range(ncat)]
