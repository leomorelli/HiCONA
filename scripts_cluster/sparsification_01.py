...
for cell in cells:
    for norm in ['KR','VC','VC_SQRT','weight','count']:
        ...
        g_filtered=disparity_filter_alpha_cut(G,weight='weight',alpha_t=0.01)
        print(cell,'| norm =',norm,':  time =',time.time()-a)
        df=nx.to_pandas_edgelist(g_filtered)
        if norm=='weight':
            df.to_csv(f'sparse_graphs/weighted_sparsification/{cell}_ICE_01.bed',sep='\t',index=None)
        else:
            df.to_csv(f'sparse_graphs/weighted_sparsification/{cell}_{norm}_01.bed',sep='\t',index=None)
