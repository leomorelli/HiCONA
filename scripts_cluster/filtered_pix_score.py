...
for cell in cells:
    ...
    pix=c.pixels()[:]
    filtered_df=pd.read_table(f'sparse_graphs/weighted_sparsification/{cell}_score_filt.bed')
    pix=pix[pix['count']>1]
    ...
    pix_new_a=pix[pix['code'].isin(filtered_df['code1'].tolist())]
    pix_new_b=pix[pix['code'].isin(filtered_df['code2'].tolist())]
    pix_new=pd.concat([pix_new_b,pix_new_a])
    pix_new=pix_new.sort_index()
    pix_new=pix_new.iloc[:,0:3]
    pix_new.to_csv(f'sparse_graphs/weighted_sparsification/{idx}_score_filt_filtered_pix.bed',sep='\t',index=None)
