...
for cell in ['IMR90','GM','HUVEC','HMEC']:
    for norm in ['ICE','KR','VC','VC_SQRT','count']:
        pix=pd.read_table(f'/shares/CIBIO-Storage/GROUPS/sharedCE/Leonardo/HiCONA/Rao_2014/sparse_graphs/weighted_sparsification/{cell}_{norm}_filtered_pix_01.bed')
        bins=pd.read_table(f'/shares/CIBIO-Storage/GROUPS/sharedCE/Leonardo/HiCONA/Rao_2014/sparse_graphs/weighted_sparsification/bins_annotated_{cell}.bed')
        ...
        for m1 in cols_marks:
            ...
            plt.savefig(f'/shares/CIBIO-Storage/GROUPS/sharedCE/Leonardo/HiCONA/Rao_2014/results/sparsification_validation/{cell}_{norm}_norm_{m1}_count_vs_distance_01.pdf')
            plt.figure()
            sns.barplot(edges,x='mark',y='abundance',hue='comp',palette=palette).set(title=f'{cell} \n Norm: {norm} \n {m1}_edges_abundance')
            plt.xticks(rotation=15, ha='right',fontsize=6)
            plt.savefig(f'/shares/CIBIO-Storage/GROUPS/sharedCE/Leonardo/HiCONA/Rao_2014/results/sparsification_validation/{cell}_{norm}_norm_{m1}_edges_abundance_01.pdf')
            plt.figure()
            sns.violinplot(edges,x='mark',y='count',hue='comp',palette=palette).set(title=f'{cell} \n Norm: {norm} \n {m1}_edges_count')
            plt.xticks(rotation=15, ha='right',fontsize=6)
            plt.savefig(f'/shares/CIBIO-Storage/GROUPS/sharedCE/Leonardo/HiCONA/Rao_2014/results/sparsification_validation/{cell}_{norm}_norm_{m1}_edges_count_01.pdf')
            plt.figure()
            sns.violinplot(edges,x='mark',y='dist',hue='comp',palette=palette).set(title=f'{cell} \n Norm: {norm} \n {m1}_edges_distance')
            plt.xticks(rotation=15, ha='right',fontsize=6)
            plt.savefig(f'/shares/CIBIO-Storage/GROUPS/sharedCE/Leonardo/HiCONA/Rao_2014/results/sparsification_validation/{cell}_{norm}_norm_{m1}_edges_distance_01.pdf')
