...
for cell in cells:
    for norm in ['weight','count']:
        ...
        if norm=='count':
            anno['count_norm']=anno['count']
        else:
            anno['count_norm']=anno['count']*anno[f'{norm}1']*anno[f'{norm}2']
        g=nx.from_pandas_edgelist(anno,source='bin1_id',target='bin2_id',edge_attr='count_norm')
        a=time.time()
        G=disparity_filter(g,weight='count_norm')
        print('step1   ',cell,'| norm =',norm,':  time =',time.time()-a)
        for p in [0.001,0.005,0.01,0.05,0.1]:
            g_filtered=disparity_filter_alpha_cut(G,weight='weight',alpha_t=p)
            print('step2   ','p=',p,cell,'| norm =',norm,':  time =',time.time()-a)
            df=nx.to_pandas_edgelist(g_filtered)
            pval=str(p).split('.')[1]
            if norm=='weight':
                df.to_csv(f'sparse_graphs/weighted_sparsification/{cell}_ICE_{pval}.bed',sep='\t',index=None)
            else:
                df.to_csv(f'sparse_graphs/weighted_sparsification/{cell}_{norm}_{pval}.bed',sep='\t',index=None)
