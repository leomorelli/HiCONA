...
for cell in cells:
    for e in exp:
        for p in pvals:
            ...
            pix=c.pixels()[:]
            filtered_df=pd.read_table(f'sparse_graphs/weighted_sparsification/{cell}_{e}_{p}.bed')
            pix=pix[pix['count']>1]
            ...
            pix_new['code']=pix_new['bin1_id'].astype(str)+'-'+pix_new['bin2_id'].astype(str)
            pix_new['code2']=[sorted(x.split('-'))[0]+'-'+sorted(x.split('-'))[1] for x in pix_new['code'].tolist()]
            pix_new=pix_new.drop_duplicates('code2')
            pix_new=pix_new.iloc[:,:3]
            pix_new.to_csv(f'sparse_graphs/weighted_sparsification/{idx}_{e}_filtered_pix_{p}.bed',sep='\t',index=None)
