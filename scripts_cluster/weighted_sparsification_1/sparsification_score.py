...
for cell in cells:
    ...
    anno['dist_scaled']=(0.9*((anno['dist_log'] - np.min(anno['dist_log'])) / (np.max(anno['dist_log']) - np.min(anno['dist_log']))))+0.1
    anno['score']=anno['dist_scaled']*anno['count']
    g=nx.from_pandas_edgelist(anno,source='bin1_id',target='bin2_id',edge_attr='score')
    a=time.time()
    G=disparity_filter(g,weight='score')
    g_filtered=disparity_filter_alpha_cut(G,weight='weight',alpha_t=0.05)
    print(cell,':  time =',time.time()-a)
    df=nx.to_pandas_edgelist(g_filtered)
    df.to_csv(f'sparse_graphs/weighted_sparsification/{cell}_score_filt.bed',sep='\t',index=None)
