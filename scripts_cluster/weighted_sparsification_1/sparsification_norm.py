...
for cell in cells:
    for norm in ['KR','VC','VC_SQRT','weight']:
        ...
        if norm=='weight':
            df.to_csv(f'sparse_graphs/weighted_sparsification/{cell}_ICE_norm.bed',sep='\t',index=None)
        else:
            df.to_csv(f'sparse_graphs/weighted_sparsification/{cell}_{norm}_norm.bed',sep='\t',index=None)
