...
def n_nodes_edges(cool_file,alpha_thr):
    ...
    for a in alpha_thr:
        ...
        df=pd.DataFrame([a,n_nodes,n_edges],index=['alpha','n_nodes','n_edges']).T
        DF=pd.concat([DF,df])
    return DF
...
def optimal_alpha(cool_file):
    ...
    while i<3:
        c=cool_file
        alpha_thr=list(np.array([x for x in range(thr_extremes[0],thr_extremes[1])])/steps[i])
        ...
        print(alpha)
        i=i+1
        thr_extremes=[int(alpha_thr[opt_a-1]*steps[i]),int(alpha_thr[opt_a+1]*steps[i])]
    return alpha,df
